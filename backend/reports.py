...
def generate_excel_report(filepath, producto_id=None, area=None, fecha_inicio=None, fecha_fin=None, tipo='completo', tipo_movimiento=None):
    wb = Workbook()
    ...
    if tipo in ['stock', 'completo']:
        if tipo == 'stock':
            ws_stock = default_sheet
        ...
        # Fetch Data
        if producto_id:
            p_data = get_producto_by_id(producto_id)
            productos = [p_data] if p_data else []
        else:
        ...
        # Headers (Stock Mínimo removed)
        headers_stock = ["ID Producto", "Nombre del Producto", "Categoría", "Stock Actual", "Estado"]
        ...
        for p in productos:
            ...
            for col_idx, val in enumerate(row_data, start=1):
                ...
                if row_fill and col_idx in [4, 5]:
                    cell.fill = row_fill
                    if col_idx == 5:
    ...
    if tipo in ['movimientos', 'completo']:
        if tipo == 'movimientos':
            ws_history = default_sheet
            ws_history.title = "Historial de Movimientos"
        else:
        ...
        # Apply filters to transaction history retrieval
        transacciones = get_transacciones(limit=10000, producto_id=producto_id, area=area, fecha_inicio=fecha_inicio, fecha_fin=fecha_fin, tipo=tipo_movimiento)
...
def generate_pdf_report(filepath, producto_id=None, area=None, fecha_inicio=None, fecha_fin=None, tipo='completo', tipo_movimiento=None):
    # Setup document
    ...
    # --- Title Page / Section ---
    if tipo == 'stock':
        story.append(Paragraph("Reporte de Stock e Inventario", style_title))
        story.append(Paragraph(f"Generado el {datetime.now().strftime('%d/%m/%Y a las %H:%M:%S')} - Estado Actual de Inventario", style_subtitle))
    elif tipo == 'movimientos':
        story.append(Paragraph("Reporte de Movimientos de Depósito", style_title))
        story.append(Paragraph(f"Generado el {datetime.now().strftime('%d/%m/%Y a las %H:%M:%S')} - Historial de Entradas y Salidas", style_subtitle))
    else:
    ...
    if tipo in ['stock', 'completo']:
        ...
        # --- Stock Table --- (Stock Mínimo removed, columns re-proportioned)
        ...
        # The 50 pt from the removed column is added to "Producto" (179 + 50 = 229)
        stock_table = Table(table_data, colWidths=[35, 229, 110, 50, 80])
    ...
    # --- Transactions Table ---
    if tipo in ['movimientos', 'completo']:
        story.append(Paragraph("Movimientos de Depósito (Filtrados)" if tipo == 'movimientos' else "Últimos Movimientos de Depósito", style_h2))
        ...
        transacciones = get_transacciones(limit=100, producto_id=producto_id, area=area, fecha_inicio=fecha_inicio, fecha_fin=fecha_fin, tipo=tipo_movimiento)
